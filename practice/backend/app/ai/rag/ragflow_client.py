...
class RAGFlowClient:
    ...
    def format_chunks_for_llm(self, chunks_data: Dict[str, Any]) -> tuple[str, List[Dict]]:
        """
        格式化chunks数据供LLM使用
        返回: (格式化的知识库文本, chunks元数据列表)
        """
        ...
        for i, chunk in enumerate(chunks, 1):
            # 为LLM格式化的知识库内容
            formatted_knowledge += f"【{i}】{chunk.get('content', '')}\n\n"
        ...
        return formatted_knowledge.strip(), chunks_metadata
